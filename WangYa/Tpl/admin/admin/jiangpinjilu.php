...
if($MOD == 'get'){
    ...
    if($ID < 1){
		...
		if(isset($_NPOST['off']) && $_NPOST['off'] > -1 ){

			$WHERE['off'] = $_NPOST['off'];
		...
		$JIANG = $D ->setbiao("jiangpin")-> where(array('id' => $DATA['jiangid'])) ->find();
		if($JIANG){

			$SHUJU['jpname'] =$JIANG['name'];
			$SHUJU['jptupian'] = pichttp( $JIANG['tupian']);
	
		}else{

			$SHUJU['jpname'] ="";
			$SHUJU['jptupian'] = pichttp("");
