...
if($MOD == 'get'){
    ...
    if($ID < 1){
        ...
        $DATA = $D -> where($WHERE) ->order("paixu desc,id desc")->limit($limit) -> select();
		$SHUJU['jine'] = array($CONN['jine'],$CONN['jifen'], $CONN['huobi']);
		$SHUJU['huodongtype'] = logac('renwutype');

		$SHUJU['gameid'] =    Game_List(1);
